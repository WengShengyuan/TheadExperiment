...
//				pool.execute(new TaskOne(i).enableExeLimit().setExeLimit(3).setInterval(1000L));
				pool.execute(new TaskOne(i, callable).enableTimeLimit().setTimeLimit(2000L).setInterval(1000L));
				Thread.sleep(10L);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		pool.execute(new TaskOne(10, callable).enableExeLimit().setExeLimit(3).setInterval(500L));
